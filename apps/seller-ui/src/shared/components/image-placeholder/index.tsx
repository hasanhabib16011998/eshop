...
const ImagePlaceHolder = ({
  size, small, onImageChange, pictureUploading, onRemove, defaultImage = null, index = null, setSelectedImage, setOpenImageModal, images
}: {
  size: string;
  small?: boolean;
  ...
  return (
    <div className={`relative ${small ? "h-[180px]" : "h-[450px]"} w-full cursor-pointer bg-[#1e1e1e] border border-gray-600 rounded-lg flex flex-col justify-center items-center`}>
      ...
      {imagePreview ? (
        ...
        <label className='absolute top-3 right-3 p-3 rounded bg-slate-700 shadow-lg cursor-pointer' htmlFor={`image-upload-${index}`}>
          <Pencil size={16} />
        </label>
      )}
